...
<Tabs>
  ...
  <Tab title="Global Installation">
    ```bash
    # Install globally
    npm install -g @artern/dbhub@latest
...
By default, DBHub attempts to install drivers for all supported databases (PostgreSQL, MySQL, MariaDB, SQL Server, SQLite, and DMDB). If you only need specific databases, you can skip the unnecessary drivers to reduce installation size.
...
npm install -g @artern/dbhub@latest --omit=optional && npm install -g pg

# Install with only PostgreSQL and MySQL drivers
npm install -g @artern/dbhub@latest --omit=optional && npm install -g pg mysql2
```
...
<Tabs>
  <Tab title="CLI">
    <Tabs>
      ...
      ```bash
      claude mcp add --transport stdio dbhub -- npx @artern/dbhub@latest --transport stdio --dsn "postgres://user:password@localhost:5432/dbname"
      ```
      ...
      ```bash
      npx @artern/dbhub@latest --transport http --port 8080 --dsn "postgres://user:password@localhost:5432/dbname"
      ```
      </Tab>
    </Tabs>
  </Tab>
  ...
  <Tab title="User Configuration">
    ...
    ```bash
    claude mcp add --scope user dbhub -- npx @artern/dbhub@latest --transport stdio --dsn "postgres://user:password@localhost:5432/dbname"
    ```
...
![Claude Desktop with DBHub](https://raw.githubusercontent.com/artern/dbhub/main/docs/images/claude-desktop.webp)
...
![Cursor with DBHub](https://raw.githubusercontent.com/artern/dbhub/main/docs/images/cursor.webp)
...
<Tabs>
  <Tab title="Interactive">
    ...
    - **Command**: `npx @artern/dbhub@latest`
...
<Tabs>
  <Tab title="CLI">
    ...
    codex mcp add dbhub -- npx @artern/dbhub@latest --transport stdio --dsn "postgres://user:password@localhost:5432/dbname"

    # Or use demo mode for testing
    codex mcp add dbhub -- npx @artern/dbhub@latest --transport stdio --demo
    ```
  </Tab>
  ...
  <Tab title="TOML">
    ...
    [mcp_servers.dbhub]
    command = "npx"
    args = ["@artern/dbhub@latest", "--transport", "stdio", "--dsn", "postgres://user:password@localhost:5432/dbname"]

    # Or use demo mode for testing
    [mcp_servers.dbhub]
    command = "npx"
    args = ["@artern/dbhub@latest", "--transport", "stdio", "--demo"]
    ```
  </Tab>
  ...
  <Tab title="Verification">
    ...
    🔌  MCP Tools
      ...
      • dbhub
        • Status: enabled
        • Auth: Unsupported
        • Command: npx -y @artern/dbhub@latest --demo
        • Tools: execute_sql
...
```bash
npx @artern/dbhub@latest --transport http --port 8080 --config /path/to/dbhub.toml
```
