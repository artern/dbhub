...
npx @artern/dbhub@latest --readonly --dsn "postgres://user:pass@localhost:5432/mydb"
```
</CodeGroup>

In read-only mode, only [allowed SQL keywords](https://github.com/artern/dbhub/blob/main/src/utils/allowed-keywords.ts) are permitted, including:
...
```bash Command Line (Deprecated)
# This flag is deprecated - use TOML configuration instead
npx @artern/dbhub@latest --max-rows 1000 --dsn "..."
```
</CodeGroup>
