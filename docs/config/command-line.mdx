...
<ParamField path="--transport" type="string" env="TRANSPORT" default="stdio">
  ...
  **Options:**
  ...
  npx @artern/dbhub@latest --transport stdio --dsn "..."

  # http - for web clients, workbench, and remote access
  npx @artern/dbhub@latest --transport http --port 8080 --dsn "..."
  ```
</ParamField>
...
<ParamField path="--port" type="number" env="PORT" default="8080">
  ...
  ```bash
  npx @artern/dbhub@latest --transport http --port 3000 --dsn "..."
  ```
</ParamField>
...
<ParamField path="DB_*" type="environment">
  ...
  |----------|------|-------------|
  | `DB_TYPE` | string | Database type: `postgres`, `mysql`, `mariadb`, `sqlserver`, `sqlite`, `dm`, `dmdb` |
  | `DB_HOST` | string | Database server hostname (not needed for SQLite) |
  | `DB_PORT` | number | Database server port. Default: PostgreSQL (`5432`), MySQL/MariaDB (`3306`), SQL Server (`1433`), DMDB (`5236`) |
...
<ParamField path="--id" type="string" env="ID">
  ...
  ```bash
  npx @artern/dbhub@latest --id prod --dsn "postgres://user:pass@prod-host:5432/db"
  npx @artern/dbhub@latest --id staging --dsn "postgres://user:pass@staging-host:5432/db"
  ```
...
<ParamField path="--demo" type="boolean" default="false">
  ...
  ```bash
  npx @artern/dbhub@latest --demo --transport http --port 8080
  ```
</ParamField>
...
<ParamField path="--config" type="string" default="./dbhub.toml">
  ...
  ```bash
  npx @artern/dbhub@latest --config ./dbhub.toml --transport http --port 8080
  ```
...
npx @artern/dbhub@latest --dsn "..." --ssh-host mybastion
